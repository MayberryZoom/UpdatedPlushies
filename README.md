# Updated Plushies for Lethal Company
A rewritten and rebalanced version of [Scintesto's Monster Plushies](https://thunderstore.io/c/lethal-company/p/Scintesto/Monster_Plushies/) with more configuration options. This mod adds various Lethal Company monsters as scrap in the form of plushies.

By default, the appearance rate of each plushie is loosely based on how common its corresponding monster is on that moon.

Tested up to v73.

Also check out [Scintesto's Scavenger Plushie](https://thunderstore.io/c/lethal-company/p/Scintesto/Scavenger_Plushie/)!

# Mod Preview
## Bracken
![Bracken Plushie](docs/assets/bracken.png)

Interact to russle its quills

## Bunker Spider
![Bunker Spider Plushie](docs/assets/bunker_spider.png)

Interact to play Minecraft spider sounds

## Coil-Head
![Coil-Head Plushie](docs/assets/coil_head.png)

Interact to make the spring go boing

## Eyeless Dog
![Eyeless Dog Plushie](docs/assets/eyeless_dog.png)

Interact to make the dog lunge

## Forest Keeper
![Forest Keeper Plushie](docs/assets/forest_keeper.png)

Interact to be scared

## Hoarding Bug
![Hoarding Bug Plushie](docs/assets/hoarding_bug.png)

Interact to YIPPEE

## Jester
![Jester Plushie](docs/assets/jester.png)

Interact to turn the crank

## Masked (Comedy/Tragedy)
![Masked (Comedy) Plushie](docs/assets/masked_comedy.png)

Interact to make it laugh

![Masked (Tragedy) Plushie](docs/assets/masked_tragedy.png)

Interact to make it cry

## Nutcracker
![Nutcracker Plushie](docs/assets/nutcracker.png)

Interact to make it investigate

## Thumper
![Thumper Plushie](docs/assets/thumper.png)

Interact to make it bite

# Credits
- Scintesto for making the original mod
- [Lethal Company modding wiki](https://lethal.wiki/)

## Original Scintesto Credits
- Evaisa for LethalLib
- Sigurd for CSync
- Malcolm for his great tutorial on Youtube that got me into lethal modding https://youtu.be/971OlPcVUHM?si=McN5FoA7ocYMxvcb
- Lethal company Modding Discord servers, Without help from the lovley folks in these servers i would have lost motavation a long time ago. If you need help with making mods then head over there and ask for help
